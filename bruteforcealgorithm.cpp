...
const double BruteforceAlgorithm::inf = 1e9;
...
void BruteforceAlgorithm::calc_dp()
{
...
void BruteforceAlgorithm::calc_order_dp()
{
    mxmask = 1 << n_orders;
    for(int msk = 0; msk < mxmask; msk++) {
        int bits = 0;
        for(int i = 0; i < n_orders; i++) bits += (msk>>i)&1;
        for(int i = 0; i < n_orders; i++) {
            if ( (msk>>i)&1 ) {
                ...
                for (int j = 0; j < n_orders; j++) {
                    if ( (msk>>j)&1 ) {
                        if ( order_dp[j][msk] + order_dist[i][j] < best) {
                            best = order_dp[j][msk] + order_dist[i][j];
                            best_j = j;
                        }
                    }
                }
...
void BruteforceAlgorithm::calc_order_dist()
{
    for(int i = 0; i < n_orders; i++) {
        for(int j = 0; j < n_orders; j++) {
...
void BruteforceAlgorithm::calc_driver_orders_dists()
{
    for(int msk = 0; msk < mxmask; msk++) {
        for(int i = 0; i < n_cafes; i++) {
            for(int j = 0; j < n_drivers; j++) {
                double new_dist = get_cafe_orders_dist(i,msk) + dist(drivers[j], cafes[i]);
...
void BruteforceAlgorithm::restore_answers()
{
...
void BruteforceAlgorithm::restore_driver(int driver)
{
...
double BruteforceAlgorithm::dist(pair<double, double> a, pair<double, double> b)
{
    return sqrt((a.first - b.first)*(a.first - b.first) + (a.second - b.second) * (a.second - b.second));
}

void BruteforceAlgorithm::init_vars()
{
...
double BruteforceAlgorithm::get_cafe_orders_dist(int cafe, int msk)
{
    double best = inf;
    for(int i = 0; i < n_orders; i++) {
        if ( (msk>>i)&1 ) {
            best = min(best, order_dp[i][msk] + dist(cafes[cafe], orders[i]));
        }
    }
    return best;
}
    ...
    for(int i = 0; i < n_orders; i++) {
        if( (msk>>i)&1 ){
            if(order_dp[i][msk] + dist(cafes[cafe], orders[i]) < best){
                best = order_dp[i][msk] + dist(cafes[cafe], orders[i]);
                best_i = i;
            }
        }
    }
